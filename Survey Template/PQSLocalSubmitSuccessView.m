...
@implementation PQSLocalSubmitSuccessView {
...
- (instancetype)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.remainOnScreenDuration = 2.5f; // set the minimum duration if there's no override
		[self hide];
...
- (void)show {
	CGRect finalFrame = self.superview.bounds;
	...
	[UIView animateWithDuration:animationDuration
					 ...
					 animations:^{
						 ...
						 [self performSelector:@selector(hide)
									withObject:self
									afterDelay:self.remainOnScreenDuration];
					 }];
}
